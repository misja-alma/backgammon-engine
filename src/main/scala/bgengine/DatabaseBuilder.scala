...
object DatabaseBuilder {
  ...
  // - a strategy for generating earlier positions from an existing one
  ...
  def build(evaluator: Position => Option[GameStatistics],
            startingPosition: Position,
            generator: Position => Seq[Position],
            database: collection.mutable.Map[String, GameStatistics],
            maxDbSize: Int): Unit = {
  ...
  def evaluate1ply(position: Position,
                   evaluator: Position => Option[GameStatistics],
                   database: collection.mutable.Map[String, GameStatistics]): GameStatistics = {
    ...
    val aggregatedStats = weightedEqs.map(_._2).reduce ( _ add _ )
    aggregatedStats.multiply(1.0 / totalWeight)
  }
}
